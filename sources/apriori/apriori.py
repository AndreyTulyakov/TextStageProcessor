...
class Apriori(Base):
    ...
    def save_to_csv_string_frequent_itemset(self):
        """Print out frequent itemset
        """
        separator = ';'
        result = 'SEQUENCE' + separator + 'SUPPORT\n'
        for key, val in self.frequent_itemset.items():
            for itemset in val:
                result += str(list(itemset)) + separator + str(round(self.frequent_itemset_support[itemset], 3)) + '\n'
        return result

    def save_to_csv_string_rules(self):
        """Print out frequent itemset
        """
        separator = ';'
        result = 'RULE' + separator + 'CONFIDENCE\n'
        for rule in self.rule:
            head = rule[0][0]
            tail = rule[0][1]
            confidence = rule[1]
            first_column = '(' + ','.join(head) + ') ==> (' + ','.join(tail) + ')'

            result += first_column + separator + str(round(confidence, 3)) + '\n'
        return result
